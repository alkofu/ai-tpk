...
3. `install.sh` copies the contents of `claude/` (agents, skills, commands, hooks, references, settings, CLAUDE.md) to `~/.claude/`, and `src/mcp/wrappers/` to `~/.claude/wrappers/`. It also registers MCP servers. Additional harnesses may be supported in future.
