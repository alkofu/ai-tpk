...
.
├── src/
│   ├── agents/          # Harness-agnostic canonical agent definitions (source of truth)
│   └── FORMAT.md        # Source format schema for agent files
├── adapters/
│   ├── to-claude.ts     # Transforms src/agents/ -> claude/agents/ (Claude Code format)
│   ├── to-opencode.ts   # Transforms src/agents/ -> opencode/agents/ (OpenCode format)
│   └── validate.ts      # Drift detection: regenerate and diff against committed files
├── claude/              # Claude Code artifacts
│   ├── agents/          # GENERATED by adapters/to-claude.ts — do not edit directly
│   ├── CLAUDE.md        # User-global instructions (hand-edited, not generated)
│   ├── settings.json    # Plugin config, hooks, marketplace settings (hand-edited)
│   ├── skills/          # Reusable capabilities (Claude Code-specific)
│   └── commands/        # Slash commands (Claude Code-specific)
├── opencode/            # OpenCode artifacts
│   ├── agents/          # GENERATED by adapters/to-opencode.ts — do not edit directly
│   └── AGENTS.md        # GENERATED global instructions for OpenCode
├── docs/                # Documentation
│   ├── CONFIGURATION.md # Detailed configuration guide
│   ├── AGENTS.md        # Agent catalog
│   ├── HARNESS_FEATURES.md # Per-harness feature availability matrix
│   └── WORKTREE_ISOLATION.md # Git worktree guide
└── install.sh           # Installation script
```

`claude/agents/` and `opencode/agents/` are **generated files**. They are committed to the repository so that `git pull` delivers correct harness-specific output without requiring contributors to run adapters. CI enforces that committed generated files match adapter output — never edit them directly. Edit `src/agents/` instead.

The Claude Code installer whitelists these paths from `claude/` into `~/.claude/`: `CLAUDE.md`, `settings.json`, `skills/`, `agents/`, and `commands/`. Anything else in the repo or on disk under `~/.claude/` is left untouched except where those destinations are replaced (after a timestamped backup).
...
./install.sh --harness claude     # Claude Code explicitly
./install.sh --harness opencode   # OpenCode (~/.config/opencode/)
./install.sh --harness all        # Both harnesses
```

Before installing, `install.sh` runs the appropriate adapter(s) to regenerate harness-specific output from `src/agents/`. This requires `deno` to be installed:

```bash
curl -fsSL https://deno.land/install.sh | sh
```

`deno` is only required when running adapters (i.e., when using `install.sh`). End users who only need the generated files can `git pull` and get correct output directly — no `deno` needed.
...
Creates symbolic links for the whitelisted Claude paths (`CLAUDE.md`, `settings.json`, `skills/`, `agents/`, `commands/`). Changes sync automatically with the repository.
...
Copies the same whitelisted Claude paths into `~/.claude/`. Manual sync required (see Updating below).
...
For per-harness feature availability, see [docs/HARNESS_FEATURES.md](/docs/HARNESS_FEATURES.md).
...
Agent definitions live in `src/agents/` as the single source of truth. The files in `claude/agents/` and `opencode/agents/` are generated by adapter scripts — do not edit them directly. Tool name mapping between harnesses (Claude Code capitalized names to OpenCode lowercase names) is documented in [`src/FORMAT.md`](/src/FORMAT.md#tool-name-mapping).
...
1. Create `src/agents/{name}.md` following the format in [`src/FORMAT.md`](/src/FORMAT.md).
2. Run `adapters/validate.ts` to verify the adapter output is correct.
3. Run `./install.sh --harness all` to regenerate generated files and install.
4. Commit both `src/agents/{name}.md` and the generated files in `claude/agents/` and `opencode/agents/`.

Never edit `claude/agents/` or `opencode/agents/` directly — they are generated and will be overwritten by adapters.

### Adding a New Harness

1. Create `adapters/to-{harness}.ts` following the pattern of the existing adapters.
2. Add a `{harness}:` extension block to `src/FORMAT.md` documenting harness-specific fields.
3. Update `install.sh` to accept `--harness {harness}` and call the new adapter.
4. Add `.gitattributes` entries marking the generated output directory as `linguist-generated=true`.

Existing adapters are unaffected — they continue to ignore the new extension block.

### Generated Files

`claude/agents/` and `opencode/agents/` are generated from `src/agents/` by the adapter scripts. They are committed so `git pull` delivers correct output without running adapters. CI runs `adapters/validate.ts` on every push and PR, failing the build if committed generated files diverge from adapter output. Do not edit generated files directly — edit `src/agents/` instead.
