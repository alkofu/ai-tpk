...
│   └── validate.ts      # Correctness checker: regenerate and verify structure
...
`claude/agents/` and `opencode/agents/` are local build artifacts (gitignored). Run `./install.sh` to generate and install them.
...
`deno` is required to run adapters. `install.sh` runs them automatically.
...
After pulling, always run `./install.sh` (or `./install.sh --copy`) to regenerate and install the generated files. They are not committed to the repository.

**With symlinks:** Run `./install.sh` after pulling.
**With copy:** Run `./install.sh --copy` after pulling.
...
4. Commit only `src/agents/{name}.md` — the generated files in `claude/agents/` and `opencode/agents/` are gitignored local artifacts.
...
`claude/agents/` and `opencode/agents/` are generated from `src/agents/` by the adapter scripts. They are gitignored local build artifacts — not committed to the repository. Run `./install.sh` to generate and install them. CI validates correctness by regenerating from source and checking structure via `adapters/validate.ts`. Edit `src/agents/` — not the generated output.
