...
├── adapters/
│   ├── to-claude.sh     # Transforms src/agents/ -> claude/agents/ (Claude Code format)
│   ├── to-opencode.sh   # Transforms src/agents/ -> opencode/agents/ (OpenCode format)
│   └── validate.sh      # Drift detection: regenerate and diff against committed files
├── claude/              # Claude Code artifacts
│   ├── agents/          # GENERATED by adapters/to-claude.sh — do not edit directly
...
│   ├── agents/          # GENERATED by adapters/to-opencode.sh — do not edit directly
...
Before installing, `install.sh` runs the appropriate adapter(s) to regenerate harness-specific output from `src/agents/`. This requires `yq` to be installed:
...
`yq` is only required when running adapters (i.e., when using `install.sh`). End users who only need the generated files can `git pull` and get correct output directly — no `yq` needed.
...
2. Run `adapters/validate.sh` to verify the adapter output is correct.
...
1. Create `adapters/to-{harness}.sh` following the pattern of the existing adapters.
...
`claude/agents/` and `opencode/agents/` are generated from `src/agents/` by the adapter scripts. They are committed so `git pull` delivers correct output without running adapters. CI runs `adapters/validate.sh` on every push and PR, failing the build if committed generated files diverge from adapter output. Do not edit generated files directly — edit `src/agents/` instead.
