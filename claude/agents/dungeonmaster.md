...
**Scope exemptions:** Bitsmith delegations that do **not** emit a `WORKING_DIRECTORY:` line do not need `EXPECTED_TREE_STATE:` either. These are: the `"## Report Write Task"` advisory `--save-report` template, the `"## Operational Execution Task"` advisory `--execute` single-command template, the `"## Operational Multi-Step Execution Task"` multi-step `/do` template, and the Worktree Creation Subroutine's step 2 inline Bash block (which is not a delegation template at all — DM runs the `git worktree add` commands directly; the exemption is structural, not templated). Do not add the field to any of these sites.
