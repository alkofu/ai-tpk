...
- **`task-observer`** — Captures lightweight session observations (friction, repeated mistakes, workarounds) as individual timestamp-and-pid-namespaced files under `~/.claude/observations/`. Observations are reviewed and turned into skill improvements via `/review-observations`, with explicit user approval on a literal unified diff before any skill file is touched. Activates by description-match; does not modify skills autonomously.
