...
| `/ask` | Ask a question about the codebase, architecture, or approach — lightweight Q&A with no planning or implementation. Routes to the Advisory Workflow (Phases A-B-C). When the session surfaces an action item, capture it with the "Action item" issue template and hand off to `/feature-issue`. |
...
| `/do <action>` | Executes an operational write action (e.g., a `gh` CLI mutation such as `gh issue label 42 bug`) via the advisory pipeline. The DM translates the user's prose into a concrete command during Phase C synthesis, presents the exact command for explicit user confirmation, and on confirmation delegates a single-shot execution to Bitsmith. No worktree, no plan file, no Ruinor review — Bitsmith executes and the outcome is reported inline. |
